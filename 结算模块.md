...
* 账务明细记录列表
   ...
   **表名：**parana_settlement_items
	...
	**业务解释：**记录每笔线上支付订单交易金额的明细即来源，例如该订单使用了多少钱优惠券、运费是多少、售中退款多少、平台抽佣等。
...
* 支付渠道汇总明细
	...
	**业务解释：**记录平台当天每笔交易金额明细，数据来源于支付模块的PayChannel，该表参与对账，该表的数据是和第三方账务数据一一对应，用该表属于与第三方账务数据进行金额核对，金额相等则说明对账成功即可标记为对账完成，服务与支付渠道日汇总。
...
* 支付渠道日汇总
   ...
   **业务解释：**记录平台当天每种支付渠道的交易金额,基于支付渠道汇总明细（已对上账务）T+1生成。
